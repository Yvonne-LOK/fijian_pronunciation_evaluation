...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build learner pronunciation feedback HTML from the latest eval run."
    )
    parser.add_argument("learner_name", help="Learner name, for example zhangle")
    parser.add_argument(
        "--eval-runs-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        help="Directory containing reference wav/txt files.",
    )
    parser.add_argument(
        "--learner-pcm-root",
        type=Path,
        default=DEFAULT_LEARNER_PCM_ROOT,
        help="Root directory containing <learner>_recordings folders.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=DEFAULT_OUTPUT_DIR,
        help="Directory to write the feedback HTML.",
    )
...
def path_to_file_uri(path: Path) -> str:
    ...
    if wsl_match:
        ...
        suffix = wsl_match.group(2)
        return f"file:///{drive}:{suffix}"
...
def find_latest_run(eval_runs_dir: Path, learner_name: str) -> tuple[str, Path]:
    ...
    for path in eval_runs_dir.iterdir():
        ...
        if not match:
            continue
        if match.group("learner") != learner_name:
            continue
...
def render_card(record: dict, reference_dir: Path, learner_dir: Path) -> str:
    pair_id = str(record.get("pair_id", ""))
    reference_id, learner_id = pair_id.split("__", 1) if "__" in pair_id else (pair_id, pair_id)
    sentence_text = (reference_dir / f"{reference_id}.txt").read_text(encoding="utf-8").strip()
    ...
    reference_audio = path_to_file_uri(reference_dir / f"{reference_id}.wav")
    learner_audio = path_to_file_uri(learner_dir / f"{learner_id}.wav")
...
def build_html(records: list[dict], learner_name: str, stamp: str, reference_dir: Path, learner_dir: Path) -> str:
    cards = "\n".join(render_card(record, reference_dir, learner_dir) for record in records)
...
def main() -> None:
    ...
    output_html = output_dir / f"{learner_name}_pronunciation_feedback.html"
    ...
    html_text = build_html(records, learner_name, stamp, reference_dir, learner_dir)
    output_html.parent.mkdir(parents=True, exist_ok=True)
    output_html.write_text(html_text, encoding="utf-8")
    print(f"[DONE] Saved pronunciation feedback HTML to: {output_html}")
