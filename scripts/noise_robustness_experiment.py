...
输出：
  - 加噪音频文件：extra_experiments/noise_robustness/audio/snr{X}/
  - 明细评分 CSV：extra_experiments/noise_robustness/results/scores_detail.csv
  - 汇总指标 CSV：extra_experiments/noise_robustness/results/summary.csv
  - 对比图：extra_experiments/noise_robustness/results/plots/*.png
"""
...
def _pick_existing(*candidates: str) -> Path:
    for c in candidates:
        p = Path(c)
        if p.exists():
            return p
    # 都不存在则返回第一个（让后续逻辑报清晰的错）
    return Path(candidates[0])


SRC_DIR = _pick_existing(
    r"D:\北外\2024-26 研究生毕设\调研\北外多语语音评测\static\fijian",
    "/mnt/d/北外/2024-26 研究生毕设/调研/北外多语语音评测/static/fijian",
)
...
def select_sentences():
    if not SRC_DIR.exists():
        raise FileNotFoundError(
            f"SRC_DIR 不存在: {SRC_DIR}\n"
            "请在脚本顶部修改 SRC_DIR 为正确路径（Windows 用 D:\\..., WSL 用 /mnt/d/...）。"
        )
    wavs = sorted(SRC_DIR.glob("u1_l1_*.wav"))
    print(f"SRC_DIR = {SRC_DIR}")
    print(f"OUT_ROOT = {OUT_ROOT}")
    print(f"found {len(wavs)} wav files matching u1_l1_*.wav")
    if len(wavs) < N_SENTENCES:
        raise ValueError(
            f"目录里只有 {len(wavs)} 个 wav 文件，少于需要的 {N_SENTENCES} 条"
        )
